      ...
      <form id="patientForm">
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <textarea class="form-control" id="medical_history" name="medical_history" rows="3" placeholder="Any relevant medical information..."></textarea>
          </div>
        ...
        <div class="row">
          <div class="col-md-6">
            <table class="table table-sm table-borderless">
              <tr><td><strong>Name:</strong></td><td id="view-name"></td></tr>
              <tr><td><strong>Gender:</strong></td><td id="view-gender"></td></tr>
              <tr><td><strong>Date of Birth:</strong></td><td id="view-dob"></td></tr>
              <tr><td><strong>Age:</strong></td><td id="view-age"></td></tr>
              <tr><td><strong>Phone:</strong></td><td id="view-phone"></td></tr>
              <tr><td><strong>Email:</strong></td><td id="view-email"></td></tr>
            </table>
          </div>
          <div class="col-md-6">
            <table class="table table-sm table-borderless">
              <tr><td><strong>Status:</strong></td><td id="view-status"></td></tr>
              <tr><td><strong>Emergency Contact:</strong></td><td id="view-emergency"></td></tr>
              <tr><td><strong>Created:</strong></td><td id="view-created"></td></tr>
              <tr><td><strong>Updated:</strong></td><td id="view-updated"></td></tr>
            </table>
          </div>
        </div>
        <div class="row">
          <div class="col-12">
            ...
            <p id="view-medical-history" class="text-muted"></p>
          </div>
        </div>
      </div>
      <div class="modal-footer">
                ...
                render: function(data) {
                    return `
                        <div class="btn-group" role="group">
                            <button class="btn btn-info btn-sm" onclick="viewRecord(${data})" title="View Details">
                                <i class="fas fa-eye"></i>
                            </button>
                            <button class="btn btn-warning btn-sm" onclick="editRecord(${data})" title="Edit Patient">
                                <i class="fas fa-edit"></i>
                            </button>
                            <button class="btn btn-danger btn-sm" onclick="deleteRecord(${data})" title="Delete Patient">
...
function viewRecord(id) {
    loadRecord('api/patients_api.php', id, function(data) {
...
function deleteRecord(id) {
    handleAjaxDelete('api/patients_api.php', id, 'patient', function() {
        $('#patientsTable').DataTable().ajax.reload();
    });
}
...
function editFromView() {
    const patientId = $('#viewPatientModal').data('current-id');
    $('#viewPatientModal').modal('hide');
    setTimeout(() => editRecord(patientId), 300);
}

function populateViewModal(patient) {
    $('#view-name').text(`${patient.first_name} ${patient.last_name}`);
    $('#view-gender').text(patient.gender ? patient.gender.charAt(0).toUpperCase() + patient.gender.slice(1) : 'Not specified');
    $('#view-dob').text(patient.date_of_birth || 'Not specified');
    $('#view-age').text(patient.date_of_birth ? `${calculateAge(patient.date_of_birth)} years` : 'Unknown');
    $('#view-phone').html(patient.phone ? `<a href="tel:${patient.phone}">${patient.phone}</a>` : 'Not provided');
    $('#view-email').html(patient.email ? `<a href="mailto:${patient.email}">${patient.email}</a>` : 'Not provided');
    $('#view-address').text(patient.address || 'Not provided');
    $('#view-emergency').text(patient.emergency_contact || 'Not provided');
    $('#view-medical-history').text(patient.medical_history || 'Not provided');
    $('#view-status').html(`<span class="badge badge-${patient.status === 'active' ? 'success' : 'secondary'}">${patient.status ? patient.status.charAt(0).toUpperCase() + patient.status.slice(1) : 'Unknown'}</span>`);
    $('#view-created').text(formatDateTime(patient.created_at));
    $('#view-updated').text(formatDateTime(patient.updated_at));
    
    // Store ID for edit button
    $('#viewPatientModal').data('current-id', patient.id);
}
